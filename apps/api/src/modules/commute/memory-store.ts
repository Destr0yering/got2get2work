...
export class MemoryCommuteStore implements CommuteStore {
  ...
  async listProfiles(tenantId: string, worksiteId: string) { return [...this.profiles.values()].filter((v) => v.tenantId === tenantId && v.worksiteId === worksiteId).map((v) => structuredClone(v)); }
