...
export class FirestoreCommuteStore implements CommuteStore {
  ...
  async listProfiles(tenantId: string, worksiteId: string) { const s = await this.db.collection("commuteProfiles").where("tenantId", "==", tenantId).where("worksiteId", "==", worksiteId).get(); return s.docs.map((d) => { const v = d.data(); return { ...v, uid: d.id, updatedAt: date(v.updatedAt) } as CommuteProfileRecord; }); }
